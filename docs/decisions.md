...
- Pull to refresh is not implemented. The offline banner, bottom sheet, splash, swipe to reveal actions on the notifications list, and full screen mobile sub views are in place. Pull to refresh was left out because TanStack Query already refetches on reconnect and the cache is dropped when the signed-in identity changes, so a manual gesture would duplicate behaviour that is reachable through the standard controls. Refetch on window focus is deliberately disabled in `providers.tsx` to keep the demo quiet.
