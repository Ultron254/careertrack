...
export function PeerFeedback() {
  ...
  if (fb.isPending) return <ViewSkeleton />;
  if (fb.isError) {
    return (
      <div className={`view ${styles.page}`}>
        <ErrorState error={fb.error} onRetry={fb.refetch} />
      </div>
    );
  }
  ...
  return (
    <div className={`view ${styles.page}`}>
      ...
      {fb.tab === 'received' && (
        <>
          {fb.received.length === 0 && !fb.receivedPending ? (
            <EmptyState
              ...
              {fb.received.map((response) => {
                ...
                return (
                  <div key={response.id} className={`card ${styles.receivedCard}`}>
                    ...
                    <div className={`${styles.blockLabel} ${styles.blockLabelStrength}`}>Strengths</div>
