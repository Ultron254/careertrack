  ...
  return (
    <>
      <div className={shared.notice}>
        ...
        <span>
          Work through each goal together. You and {managerFirst} both confirm a final number
          before it locks.
        </span>
      </div>
          ...
          return (
            ...
            >
              <div className={styles.sectionHead}>
                ...
                {settled && (
                  <span className={`${shared.statePill} ${styles.pillRight} ${shared.statePillLocked}`}>
                    ✓ Locked
                  </span>
                )}
                {state.status === 'flagged' && (
                  <span className={`${shared.statePill} ${styles.pillRight} ${shared.statePillFlagged}`}>
                    Flagged · People Team
                  </span>
                )}
              </div>
              ...
              <div className={styles.duelFoot}>
                <span className={shared.discussHint} style={{ margin: 0 }}>
                  ...
                  {state.status === 'locked' &&
                    `You and ${managerFirst} agreed on ${state.value}.`}
                  {state.status === 'flagged' &&
  ...
  return (
    <div className={`card ${styles.blockCard}`}>
      ...
      <p className={styles.signIntro}>
        All {goalCount} goals are aligned. Each party signs to confirm the discussion took place
        and the ratings are agreed. The People Team signs last to lock the record.
      </p>
      ...
      <div className={`card ${styles.doneCard}`}>
        ...
        <p className={styles.doneBody}>
          ...
          {final > 0 && (
            <>
              {' '}
              Final overall rating <strong>{final.toFixed(1)}</strong> {'\u00b7'} {ratingWord(final)}.
            </>
          )}
        </p>
      </div>
