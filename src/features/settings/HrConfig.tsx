...
const ratingScale = [
  { n: 1, label: 'Does not meet expectations', def: 'Performance is consistently below the required standard; significant improvement needed.' },
  { n: 2, label: 'Meets some expectations', def: 'Meets some but not all requirements; development needed in key areas.' },
  { n: 3, label: 'Meets expectations', def: 'Consistently delivers against goals and behaves in line with our values.' },
  { n: 4, label: 'Exceptional', def: 'Consistently exceeds goals and role expectations; a role model for others.' },
] as const;
...
const escalationCopy: Record<EscalationRule, { title: string; desc: string; colour: string }> = {
  notify_manager: { title: 'Notify line manager', desc: 'Alert the manager the same day.', colour: 'var(--blue)' },
  notify_people_team: { title: 'Escalate to People Team', desc: 'Add to the late list after 2 days.', colour: 'var(--orange)' },
  auto_extend: { title: 'Auto extend deadline', desc: 'Grant a 3 day grace window automatically.', colour: 'var(--teal)' },
  flag_record: { title: 'Flag on record', desc: 'Note the miss on the cycle record.', colour: 'var(--gold)' },
};
    ...
    <>
      ...
      <div>
        {section === 'categories' && (
          <>
            ...
            <div className={`card ${styles.card}`}>
              <div className={styles.catGrid}>
                ...
                {draft.categories.map((c) => (
                  <div key={c.category} style={{ display: 'contents' }}>
                    <span className={styles.catSwatch} style={{ background: categoryColour[c.category] }} />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifySelf: 'center' }}>
                      <input
                        ...
                        onChange={(e) =>
                          editor.setCategoryWeight(c.category, Number(e.target.value.replace(/[^0-9]/g, '')) || 0)
                        }
                      />
              ...
              <div className={styles.catFooter}>
                <span
                  ...
                  style={{ color: editor.totalWeightPct === 100 ? 'var(--status-approved-fg)' : 'var(--status-returned-fg)' }}
                >
            ...
            <div className={styles.note}>
              ...
              <span>Employees set 4 to 5 goals, at least one per required category, with at least one stretch goal.</span>
            </div>
          </>
        )}
        ...
        {section === 'reviews' && (
          <>
            <div className={styles.sectionHead}>
              ...
              <p className={styles.sectionSub}>Which appraisal stages apply, and whether peer feedback feeds in.</p>
            </div>
            ...
            <div className={`card ${styles.card}`}>
              {draft.cyclePhases.map((phase, index) => (
                <div key={phase.name} className={styles.phaseRow}>
                  <span className={styles.phaseDot} style={{ background: `var(--rating-${(index % 4) + 1})` }} />
        ...
        {section === 'reminders' && (
          <>
            <div className={styles.sectionHead}>
              ...
              <p className={styles.sectionSub}>Reminder cadence and what happens when employees are late.</p>
            </div>
            <div className={`card ${styles.card}`}>
            ...
            <div className={`card ${styles.card}`}>
              ...
              {draft.escalations.map((e, index) => (
                <div
                  ...
                  style={{ border: '1px solid var(--border)', borderRadius: 13, marginBottom: 10 }}
                >
                  <span className={styles.escStep} style={{ background: escalationCopy[e.rule].colour }}>
                    {index + 1}
                  </span>
                  <div className={styles.rowBody}>
        ...
        {section === 'adhoc' && (
          <>
            <div className={styles.sectionHead}>
              ...
              <p className={styles.sectionSub}>Allow goals outside the submission window, with conditions.</p>
            </div>
            <div className={`card ${styles.card}`}>
        ...
        <div className={styles.saveBar}>
          <button type="button" className={styles.saveButton} onClick={editor.save} disabled={editor.saving}>
