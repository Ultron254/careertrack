...
export function AppraisalPeerPanel({ a }: { a: ReturnType<typeof useSelfAppraisal> }) {
  ...
  const colleagues = useMemo(() => {
    if (!deptId) return [];
    return [...a.usersById.values()].filter((u) => u.departmentId === deptId && u.id !== user?.id);
  }, [a.usersById, deptId, user?.id]);
      ...
      <div className={`card ${styles.peerCard}`}>
        ...
        <div className={styles.peerRequestRow}>
          ...
          <button
            ...
            disabled={!peerId || atLimit || sending}
          >
            {sending ? 'Sending' : 'Request appraisal'}
          </button>
        </div>
        {atLimit && (
        ...
        <div className={styles.peerList}>
          {a.received.map((response) => {
            ...
            const deptName = author?.departmentId
              ? deptNameById.get(author.departmentId)
              : undefined;
            return (
              <div key={response.id} className={styles.peerRow}>
                <div className={styles.peerRowHead}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className={styles.peerDept}>
                      {deptName ?? author?.jobTitle ?? 'Colleague'}
                    </div>
                  </div>
                ...
                {response.strengths && (
                  <div className={styles.peerComment}>{response.strengths}</div>
                )}
              </div>
            );
          })}
              ...
              <div key={requestItem.id} className={styles.peerRow}>
                <div className={styles.peerRowHead}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className={styles.peerDept}>
                      {deptName ?? peer?.jobTitle ?? 'Colleague'}
                    </div>
                  </div>
