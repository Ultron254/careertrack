...
import type { AuditEvent, Cycle, User } from '@/Types/domain';
import type { HrConfig as HrConfigData } from '@/Types/hrConfig';
...
const basePrefs: NotificationPref[] = [
  {
    id: 'goal-status',
    label: 'Goal returned or approved',
    desc: 'When a manager approves or returns your goals.',
    on: true,
  },
  {
    id: 'peer-feedback',
    label: 'Peer feedback requests',
    desc: 'When a colleague asks you for feedback.',
    on: true,
  },
  {
    id: 'review-meetings',
    label: 'Upcoming review meetings',
    desc: 'Reminders before a scheduled review.',
    on: true,
  },
  {
    id: 'deadline',
    label: 'Cycle deadline reminders',
    desc: 'Before each phase of the cycle closes.',
    on: true,
  },
  {
    id: 'digest',
    label: 'Weekly digest email',
    desc: 'A Monday summary of what needs your attention.',
    on: false,
  },
];
...
const hrPrefs: NotificationPref[] = [
  {
    id: 'goal-status',
    label: 'Goal returned or approved',
    desc: 'Individual goal decisions across the org.',
    on: false,
  },
  {
    id: 'escalations',
    label: 'Escalations & overdue submissions',
    desc: 'When submissions slip past their SLA.',
    on: true,
  },
  {
    id: 'org-digest',
    label: 'Weekly org digest',
    desc: 'Cycle progress across every department.',
    on: true,
  },
  {
    id: 'individual-alerts',
    label: 'Individual submission alerts',
    desc: 'Every single goal submission as it lands.',
    on: false,
  },
  {
    id: 'system-audit',
    label: 'System & audit alerts',
    desc: 'Access exceptions and configuration changes.',
    on: true,
  },
];
...
export function Settings({ users, auditLog, config, cycles }: SettingsProps) {
  ...
  return (
    <div className={`view ${styles.page}`}>
      ...
      {tab === 'profile' && <ProfileTab user={user} users={users} auditLog={auditLog} />}
      {tab === 'config' && canConfigure && <HrConfig config={config} cycles={cycles} />}
      {tab === 'notifications' && (
